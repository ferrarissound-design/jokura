...
function generateEpicSpires(){
  ...
  try{
    ...
    for(let i=0;i<nSp;i++){
      ...
      // アーチ塔はリーン弱め: 頂部が門の真上からずれてゲートが崩れるのを防ぐ
      const leanAmp=isArch?.14:.3;
      spires.push({
        bx:cx0+Math.cos(ang)*r,bz:cz0+Math.sin(ang)*r,
        H:Math.round(isArch?24+Math.random()*4:12+Math.random()*10),
    ...
    for(const s of spires){
      ...
      for(let t=0;t<=H;t++){
        const y=ybase+t;
        // 先細り＋ノイズで有機的な輪郭、リーンの累積でゆるく傾く
        const rr=Math.max(1,s.r0*(1-0.65*t/H)+noise(px*.3,t*.4)*.6);
    ...
    if(archTops.length===2){
      const[a,b]=archTops;
      const mx=(a[0]+b[0])/2,my=Math.min((a[1]+b[1])/2+5,yTop),mz=(a[2]+b[2])/2;
      for(let i=0;i<=40;i++){
        const t=i/40,u=1-t;
    ...
    for(const s of spires){
      ...
      for(let li=0;li<nL;li++){
        ...
        const y=ybase+t;
        const rr=Math.max(1,s.r0*(1-0.65*t/H))+2;
      ...
      for(let t=2;t<H;t++){
        ...
        const y=ybase+t;
        const rr=Math.max(1,s.r0*(1-0.65*t/H));
    ...
    // ─ 5. 巨木: 中央に 2×2 幹＋斜め枝＋楕円体の樹冠 ─
    const TH=11+Math.floor(Math.random()*4);
    for(let t=1;t<=TH;t++)for(let dx=0;dx<=1;dx++)for(let dz=0;dz<=1;dz++)
      put(cx0+dx,ybase+t,cz0+dz,3);
    const canY=ybase+TH;
    for(let bi=0;bi<4;bi++){ // 枝: 4方向へ斜め上に伸びる
      const ba=Math.PI/4+bi*Math.PI/2+(Math.random()-.5)*.5;
      let bx=cx0+.5,bz=cz0+.5,by=canY-3-Math.floor(Math.random()*2);
    ...
    const pondX=Math.round(cx0-fx*5),pondZ=Math.round(cz0-fz*5),pondR=3.5+Math.random()*.5;
    for(let dx=-4;dx<=4;dx++)for(let dz=-4;dz<=4;dz++){
      const d2=dx*dx+dz*dz;if(d2>(pondR+1)*(pondR+1))continue;
      const x=pondX+dx,z=pondZ+dz;
      if(Math.hypot(x-cx0,z-cz0)>R-1)continue; // 土台の外へはみ出さない
      if(d2<=pondR*pondR){
        put(x,ybase-1,z,2);
        put(x,ybase,z,WATER_BLOCK);
      }else{
        put(x,ybase,z,2); // 砂の縁
      }
    }
