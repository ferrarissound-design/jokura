...
// 人型の亡骸」をプロシージャルに生成する。頭蓋骨・背骨・肋骨・両腕と手・脚・胸の
...
// _grLocalToWorld）でワールド座標へ変換する。yaw だけなので球は回転不変となり、
...
function _grSetProgress(show,frac){
  const el=document.getElementById('worldTreeProgress');if(!el)return;
  const lbl=document.getElementById('wtpLabel');
  el.style.display=show?'':'none';
  if(show){if(lbl)lbl.textContent='🦴 巨人の亡骸を生成中…';const fill=document.getElementById('wtpFill');if(fill)fill.style.width=Math.round((frac||0)*100)+'%';}
  else if(lbl)lbl.textContent='🌳 世界樹を生成中…';
}

// getTerrainHeight: 既存の地表高さ取得のエイリアス（各部位の接地に使う）
function _grGroundY(x,z){return surfaceHeightAt(x,z);}

// rotateLocalPosition: ローカル座標→ワールド座標（鉛直yaw回転＋スケール＋アンカー平行移動）
function _grLocalToWorld(plan,lx,ly,lz){
...
function _grSphere(plan,wx,wy,wz,r,opts){
...
function _grSpawnEnemyAt(wx,wy,wz,idx){
  ...
  }catch(e){console.warn('巨人の亡骸: 敵の配置に失敗',e);}
}
...
function _grPlaceChest(wx,wy,wz){
  ...
  if(!openedTreasureKeys.has(tk))scene.add(mesh);
  underTreasures[tk]={mesh,opened:openedTreasureKeys.has(tk),type:3,struct:'giantRemains'};
}
...
function _grSpine(plan){
  ...
  for(let x=x0;x<=x1;x+=seg){
    ...
    const w=_grLocalToWorld(plan,x,y,z);
    plan.contacts.push({x:w.x,y:w.y,z:w.z});
    if(_wtHash((w.x*31)^(w.z*17))<plan.damageRate*0.6)continue; // 一部が崩れて途切れる
    _grSphere(plan,w.x,w.y,w.z,Math.max(1,boneMul(plan,1.15)),{});
    // 椎骨の棘突起（上へ突き出す小さな節）で「棒でない骨らしさ」を出す
    _grSphere(plan,w.x,w.y+Math.round(plan.boneR*1.2*plan.scale),w.z,Math.max(1,boneMul(plan,0.5)),{});
  }
}
...
function _grRibCage(plan){
  ...
  for(let i=0;i<cfg.ribCount;i++){
    ...
    for(const side of[-1,1]){
      ...
      _grPolyline(plan,pts,plan.boneR*0.95,{damage:plan.damageRate*0.4});
    }
    // 胸骨: 肋骨の頂点付近を体軸方向につなぐ節（心核の上だけは割れ目として開ける）
    if(Math.abs(ribX-plan.chestX)>3.0){
      const w=_grLocalToWorld(plan,ribX,plan.sternumY,0);
      _grSphere(plan,w.x,w.y,w.z,Math.max(1,boneMul(plan,0.9)),{damage:plan.damageRate*0.5});
    }
    // 肋骨内部の敵の湧き位置候補（床＝地表付近）
    if(i%3===1){const w=_grLocalToWorld(plan,ribX,plan.spineY,0);plan.enemySpots.push({x:w.x,y:plan.ybase,z:w.z});}
  }
  // 肋骨内部に1つ宝箱候補（胸郭のホール床）
  const wc=_grLocalToWorld(plan,plan.shoulderX+cfg.ribSpacing,plan.spineY,plan.ribWidth*0.4);
...
function _grSkull(plan){
  const skullR=plan.skullR,cy=skullR+plan.spineY*0.2; // ローカルの頭中心の高さ
  const S=_grLocalToWorld(plan,0,cy,plan.headZ);      // 頭中心（ワールド）
  plan.skullWorld=S;
  const thk=Math.max(2,Math.round(plan.boneR*plan.scale));
  // 頭蓋骨の殻（後頭部側=+x に欠けを作る）
  _grSphere(plan,S.x,S.y,S.z,skullR*plan.scale,{thk,damage:plan.damageRate*0.35});
  _grCarve(S.x,S.y,S.z,(skullR-thk-0.5)*plan.scale);   // 内部を確実に空洞化
  ...
  for(let dx=-fr;dx<=fr;dx++)for(let dz=-fr;dz<=fr;dz++)if(dx*dx+dz*dz<=fr*fr)put(S.x+dx,floorY,S.z+dz,_grBone(S.x+dx,floorY,S.z+dz,plan));
  // 顔は体と反対（-x側）。目・鼻・口・後頭部のひびを掘って侵入口にする
  const F=v=>_grLocalToWorld(plan,v[0],cy+v[1],plan.headZ+v[2]);
  const eyeL=F([-skullR*0.55,skullR*0.15,-skullR*0.42]),eyeR=F([-skullR*0.55,skullR*0.15,skullR*0.42]);
  _grCarve(eyeL.x,eyeL.y,eyeL.z,2.3*plan.scale);      // 目の空洞（侵入口）
  _grCarve(eyeR.x,eyeR.y,eyeR.z,2.3*plan.scale);
  const nose=F([-skullR*0.72,-skullR*0.02,0]);_grCarve(nose.x,nose.y,nose.z,1.4*plan.scale); // 鼻の空洞
  const mouth=F([-skullR*0.5,-skullR*0.5,0]);_grCarve(mouth.x,mouth.y,mouth.z,2.6*plan.scale); // 口（侵入口）
  const crack=F([skullR*0.7,skullR*0.25,0]);_grCarve(crack.x,crack.y,crack.z,2.2*plan.scale);  // 後頭部のひび（侵入口）
  const chip=F([skullR*0.35,skullR*0.6,skullR*0.2]);_grCarve(chip.x,chip.y,chip.z,2.0*plan.scale); // 頭頂部の欠け
  // 苔・土の付着（上半球に少量）
  for(let n=0;n<20;n++){
...
function _grJaw(plan,cy){
  ...
  _grPolyline(plan,pts,plan.boneR*0.85,{damage:plan.damageRate*0.4});
}

// ── generateArm + generateHand: 左右で異なる姿勢（片方は地面へ伸び、片方は肘を曲げる）。
// 上腕・前腕・肘、そして指を1本ずつ判別できる巨大な手。手のひらに報酬を置く ──
function _grArm(plan,side){
  ...
  _grSphere(plan,..._triW(plan,shoulder),Math.max(1,boneMul(plan,1.6)),{}); // 肩関節
  _grBoneSeg(plan,shoulder,elbow,plan.boneR*1.4,plan.boneR*1.1,{});          // 上腕
  _grSphere(plan,..._triW(plan,elbow),Math.max(1,boneMul(plan,1.3)),{});     // 肘
  _grBoneSeg(plan,elbow,wrist,plan.boneR*1.1,plan.boneR*0.9,{});             // 前腕
  _grHand(plan,side,wrist,elbow,bent);
}
// ローカル点→_grLocalToWorld の (x,y,z) を配列で返す（スプレッド用）
function _triW(plan,p){const w=_grLocalToWorld(plan,p[0],p[1],p[2]);return[w.x,w.y,w.z];}
function _grHand(plan,side,wrist,elbow,bent){
  ...
  // 手のひら（平たい塊）
  _grSphere(plan,..._triW(plan,palm),Math.max(1,plan.boneR*1.8*hs*plan.scale),{});
  ...
  for(let f=0;f<5;f++){
    ...
    for(let s=0;s<nSeg;s++){
      ...
      _grBoneSeg(plan,a,b,(1.0-t0*0.4)*plan.boneR*0.7*hs,(1.0-t1*0.4)*plan.boneR*0.7*hs,{});
      _grSphere(plan,..._triW(plan,a),Math.max(1,plan.boneR*0.55*hs*plan.scale),{}); // 指の関節
    }
  }
  // 手のひらの上に報酬（伸ばした手＝発光する石／曲げた手＝古代の紋章風の鉱石）＋宝箱候補
  const pw=_grLocalToWorld(plan,palm[0],palm[1]+1,palm[2]);
...
function _grLeg(plan,side){
  ...
  // 骨盤の塊
  _grSphere(plan,..._triW(plan,[plan.pelvisX,plan.spineY+1,0]),Math.max(1,boneMul(plan,2.0)),{});
  _grSphere(plan,..._triW(plan,hip),Math.max(1,boneMul(plan,1.6)),{});      // 股関節
  _grBoneSeg(plan,hip,knee,plan.boneR*1.6,plan.boneR*1.2,{damage:buried?plan.damageRate:0}); // 大腿骨
  _grSphere(plan,..._triW(plan,knee),Math.max(1,boneMul(plan,1.4)),{});     // 膝
  _grBoneSeg(plan,knee,ankle,plan.boneR*1.2,plan.boneR*1.0,{damage:buried?plan.damageRate*1.5:0}); // すね
  if(!buried){                                                              // 足＋足の指（埋没脚は省略）
    _grBoneSeg(plan,ankle,foot,plan.boneR*1.1,plan.boneR*1.3,{});
    for(let t=0;t<5;t++){
      const toe=[foot[0]+2+t*0.3,foot[1]-0.3,foot[2]+(t-2)*1.1*side];
      _grBoneSeg(plan,foot,toe,plan.boneR*0.6,plan.boneR*0.35,{damage:plan.damageRate});
    }
...
function _grHeartCore(plan){
  const cx=plan.chestX,coreY=plan.sternumY*0.5,roomR=Math.max(4,plan.skullR*0.6);
  const C=_grLocalToWorld(plan,cx,coreY,0);
  // 古代石の球殻の部屋（黒曜石＝古代石）。内部は空洞
  _grSphere(plan,C.x,C.y,C.z,roomR*plan.scale,{ti:OBSIDIAN_BLOCK,thk:1.6});
  _grCarve(C.x,C.y,C.z,(roomR-1.6)*plan.scale);
  // 床
  ...
  // 中心核: 宙に浮く発光する結晶塊
  _grSphere(plan,C.x,C.y,C.z,Math.max(1.5,2.2*plan.scale),{ti:CRYSTAL_BLOCK});
  // 胸の割れ目（外壁の -y 側＝胸の外側から内部へ続く侵入口）。同時に光が漏れる
  const slit=_grLocalToWorld(plan,cx,plan.sternumY,0);
  _grCarve(slit.x,slit.y,slit.z,2.4*plan.scale);
  put(slit.x,slit.y-1,slit.z,CRYSTAL_BLOCK); // 割れ目のすぐ内側に発光ブロック→外からわずかに光る
  // 側面からの侵入口（肋骨の隙間からまっすぐ心核へ）
  const ent=_grLocalToWorld(plan,cx,coreY,plan.ribWidth);
...
function _grWeapon(plan){
  // 伸ばした腕（=曲げていない側）の手の先あたりに配置
  const side=-plan.bentArm,armL=plan.cfg.armLength;
  const hand=_grLocalToWorld(plan,plan.shoulderX+armL*0.75,plan.spineY,side*(plan.ribWidth+armL*0.62));
  const gx=hand.x+Math.round((plan.rng()*6-3)),gz=hand.z+Math.round((plan.rng()*6-3));
  const gy=_grGroundY(gx,gz);
  ...
  for(let dx=-Math.round(8*S);dx<=Math.round(8*S);dx++)for(let dz=-Math.round(8*S);dz<=Math.round(8*S);dz++){
    const d=Math.hypot(dx,dz);if(d>8*S)continue;const x=gx+dx,z=gz+dz,sh=_grGroundY(x,z);
  ...
  // 根元の小さな探索空間＋宝箱候補
  _grCarve(gx,gy,gz,2.2*S);put(gx,gy-1,gz,1);
...
function _grDebris(plan){
  ...
  for(let i=0;i<n;i++){
    ...
    const x=plan.cx0+Math.round(Math.cos(a)*d),z=plan.cz0+Math.round(Math.sin(a)*d),sh=_grGroundY(x,z);
    const r=1+Math.floor(plan.rng()*2);
    for(let dx=-r;dx<=r;dx++)for(let dz=-r;dz<=r;dz++)for(let dy=0;dy<=r;dy++){if(dx*dx+dy*dy+dz*dz>r*r)continue;if(voxels[vKey(x+dx,sh+dy,z+dz)]&&plan.rng()<0.5)continue;put(x+dx,sh+dy,z+dz,_grBone(x+dx,sh+dy,z+dz,plan));}
  }
}

// ── blendWithTerrain: 接地点の下に土/石を詰めて骨が浮かないようにし、上に苔・土を少量
// 載せて地形の一部に見せる。地面の高さを取得して各接地点に合わせて盛る ──
function _grBlend(plan){
  const mr=4;
  for(const c of plan.contacts){
    for(let dx=-mr;dx<=mr;dx++)for(let dz=-mr;dz<=mr;dz++){
      const d=Math.hypot(dx,dz);if(d>mr)continue;
      const x=c.x+dx,z=c.z+dz,sh=_grGroundY(x,z);
...
function _grTreasure(plan){
  const spots=plan.chestSpots;let placed=0;
  for(let i=0;i<spots.length&&placed<plan.cfg.treasureCount;i++){const s=spots[i];_grPlaceChest(s.x,s.y,s.z);placed++;}
}
// placeEnemies: 集めた候補位置から enemyCount 体の敵を配置（既存の敵システム）。
// クリエイティブでは湧くだけ。心核のボス候補には強めの敵（ゴーレム系）を置く
function _grEnemies(plan){
  ...
  for(let i=0;i<spots.length&&placed<plan.cfg.enemyCount;i++){
    ...
    _grSpawnEnemyAt(s.x,s.y,s.z,idx);placed++;
  }
}

let _giantRemainsBusy=false; // 生成中フラグ（ボタン連打の多重生成を防ぐ）
function generateGiantRemains(){
  if(_giantRemainsBusy){showBonus('🦴 巨人の亡骸を生成中…');return;}
  if(!window.confirm('超巨大な「巨人の亡骸」を生成します。周囲の地形が変化する場合があります。生成しますか？'))return;
  _giantRemainsBusy=true;_grSetProgress(true,0);
  let plan;
  try{
    const anchor=_frontAnchor(GIANT_REMAINS_CFG.anchorDist);
    const cfg=GIANT_REMAINS_CFG;
    ...
    console.error('巨人の亡骸: 準備中にエラー',e);
    _giantRemainsBusy=false;_grSetProgress(false);showBonus('⚠ 巨人の亡骸の生成に失敗しました');return;
  }
  ...
  const phases=[
    ...
    ()=>{_grArm(plan,-1);},
    ()=>{_grArm(plan,1);},
    ()=>{_grLeg(plan,-1);_grLeg(plan,1);},
    ()=>_grHeartCore(plan),
  ];
  if(GIANT_REMAINS_CFG.generateWeapon)phases.push(()=>_grWeapon(plan));
  phases.push(()=>{_grDebris(plan);_grBlend(plan);});
  phases.push(()=>{_grTreasure(plan);_grEnemies(plan);});
  let idx=0;_deferDirty=true;
  const step=()=>{
    try{
      phases[idx]();idx++;
      _deferDirty=false;flushDirtyChunks();
      _grSetProgress(true,idx/phases.length);
      if(idx<phases.length){_deferDirty=true;requestAnimationFrame(step);}
      else{
        _giantRemainsBusy=false;_grSetProgress(false);showBonus('🦴 巨人の亡骸を生成！');
        playTone(196,.16,.1,'triangle');setTimeout(()=>playTone(262,.16,.1,'triangle'),150);setTimeout(()=>playTone(330,.2,.1,'triangle'),320);
      }
    }catch(e){
      console.error('巨人の亡骸: 生成中にエラー',e);
      _deferDirty=false;try{flushDirtyChunks();}catch(_){}
      _giantRemainsBusy=false;_grSetProgress(false);showBonus('⚠ 巨人の亡骸の生成に失敗しました');
    }
  };
  requestAnimationFrame(step);
}
...
const SPECIAL_STRUCTURES=[
  ...
  {key:'giantRemains',icon:'🦴',label:'巨人の亡骸',desc:'倒れて石化した超巨大な人型の亡骸。頭蓋骨・肋骨・心核を探索',fn:generateGiantRemains},
];
