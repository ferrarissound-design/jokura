...
function killBoss(){
  ...
  sfxBossDie();showBonus('💀 BOSS DEAD! +'+bossScore);
...
function updateHUD(){
  ...
  const isBossNext=BOSS_DEFS.some(b=>b.wave===gs.wave+1);
  if(gs.nextWave>0&&gs.nextWave<=10){
    $wt.textContent=(isBossNext?'👑 BOSS WAVE ':'⚠️ WAVE ')+(gs.wave+1)+' まで '+Math.ceil(gs.nextWave)+'秒';
    $wt.className='show'+(isBossNext?' boss':'');
